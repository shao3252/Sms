...
window.appFeatures = {
    ...
    renderMyReports: async () => {
        ...
        window.router.navigate('my-reports');
    },
    ...
    submitAppeal: async () => {
        ...
        if(text) {
            ...
            window.ui.hideModal('appeal-modal');
            window.ui.showToast('Maombi yametumwa', 'success');
        }
    },
    ...
    deleteNotification: async (notifId) => { 
        ...
        window.ui.showToast('Imefutwa', 'success'); 
    },
    renderNotifications: () => { 
    ...
    handleNotificationClick: async (type, linkId, notifId) => { 
        ...
        if(type === 'user' && linkId) window.appFeatures.viewUserProfile(linkId); 
        else if(type === 'post' && linkId) { 
            const s = await getDoc(doc(db, "posts", linkId)); 
            if(s.exists()) { 
                document.getElementById('single-post-container').innerHTML = window.appFeatures.createPostHTML({id: s.id, ...s.data()}); 
                window.router.navigate('single-post'); 
            } 
        } 
    }
};
...
window.appAdmin = {
    ...
    postAnnouncement: async () => { 
        ...
        // Refresh feed ili lionekane juu haraka
        if(window.appFeatures.renderAnnouncementsToFeed) {
            window.appFeatures.renderAnnouncementsToFeed();
        }
    },
    ...
    deleteAnnouncement: async (id) => { 
        if(confirm("Delete?")) { 
            ...
            // Refresh pia matangazo ya kule juu kwenye Feed
            if(window.appFeatures.renderAnnouncementsToFeed) {
                window.appFeatures.renderAnnouncementsToFeed();
            }
        } 
    ...
    renderAppeals: async () => {
        ...
        if(!found) {
            area.innerHTML += "<p style='text-align:center;'>Hakuna maombi ya kufunguliwa block.</p>";
        }
    },
    ...
    resolveAppeal: async (appealId, userId, unblock) => { 
        ...
        window.ui.showToast(unblock ? 'Amefunguliwa Block' : 'Imekataliwa', 'success'); 
        window.appAdmin.renderAppeals(); 
    }
};
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    if(currentUser) {
        ...
        if(currentUser.role === 'admin') {
            ...
            const premOpt = document.getElementById('opt-premium');
            if(premOpt) premOpt.style.display = 'block'; 
        }
        
        window.appFeatures.renderFeed(); 
        
        // HAPA NDIO ANNOUNCEMENTS ZINAANZISHWA KUFANYA KAZI (ZIKIZUNGUKA)
        window.appFeatures.renderAnnouncementsToFeed();
